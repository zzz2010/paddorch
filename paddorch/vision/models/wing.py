...
class AddCoordsTh(nn.Module):
    ...
    def forward(self, x, heatmap=None):
        ...
        """
        coords = self.coords.repeat(x.shape[0], 1, 1, 1)
...
def preprocess(x):
    ...
    for part, ops in operations.items():
        ...
        torch.copy( resize(shift(x[:, start:end], ops.shift), ops.resize),x[:, start:end])
         # =
    ...
    x2.fill_(0, 1, list(range(index_map.lipedges.start, index_map.lipedges.end)))
    x2.fill_(0, 1, list(range(index_map.eyebrows.start, index_map.eyebrows.end)))
