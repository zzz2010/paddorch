...
def calculate_kid_given_paths(paths, batch_size, cuda,   model_path='inception',dims=2048):
    ...
    for j, pth in enumerate(pths):
        print(paths[j+1])
        actj = _compute_activations(pth, model, batch_size, dims, cuda, model_path)
...
def polynomial_mmd_averages(codes_g, codes_r, n_subsets=50, subset_size=1000,
    ...
    with tqdm(range(n_subsets), desc='MMD', file=output) as bar:
        for i in bar:
            g = codes_g[choice(codes_g.shape[0], min(codes_g.shape[0],subset_size), replace=False)]
            r = codes_r[choice( codes_r.shape[0], min(codes_r.shape[0],subset_size), replace=False)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='../../metrics/inception_v3_pretrained.pdparams', type=str,
                        help='inception pretrained model path')
    ...
    with fluid.dygraph.guard(place=place):
        results = calculate_kid_given_paths(paths, args.batch_size, args.gpu != '',   model_path=args.model)
        for p, m, s in results:
            print('KIDx100 (%s): %.6f (%.6f)' % (p, m*100, s*100))