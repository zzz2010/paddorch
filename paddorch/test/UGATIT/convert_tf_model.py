...
def load_tf_pretrain_model(paddle_model, tf_state_dict):
    ...
    for torch_key, paddle_key in zip(torch_key_list,paddle_weight.keys()):
        ...
        if ("fc" in torch_key and "weight" in torch_key) or (len(tf_state_dict[torch_key].shape) == 2 and tf_state_dict[torch_key].shape[0] == tf_state_dict[torch_key].shape[1]):
            new_weight_dict[paddle_key] = tf_state_dict[torch_key].T
        elif int(paddle_weight[paddle_key].shape[-1])==int(tf_state_dict[torch_key].shape[-1])  :
            new_weight_dict[paddle_key]=tf_state_dict[torch_key]
        else:
            new_weight_dict[paddle_key] = tf_state_dict[torch_key].T
...
genA2B = ResnetGenerator(input_nc=3, output_nc=3, ngf=64, n_blocks=4, img_size=256, light=False)
...
print("tf weight items:",len(tf_state_dict_split_weight_bias))
with fluid.dygraph.guard(place=place):
    load_tf_pretrain_model(genA2B, tf_state_dict_split_weight_bias)